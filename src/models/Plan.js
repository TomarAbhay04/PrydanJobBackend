...
//     // Duration in days. Will default based on plan name (see pre-save).
//     duration: {
//       type: Number,
...
// // Pre-save: set sensible defaults when missing or when creating from name
// planSchema.pre('save', function (next) {
//   // if name present and duration not explicitly set, apply default duration
//   if (this.name && (!this.duration || this.isNew)) {
//     const def = DEFAULT_DURATIONS[this.name];
//     if (def) this.duration = def;
//   }

//   // ensure features is an array (avoid required forcing on single string)
...
// // Simple helper to get plan by name (case-insensitive)
// planSchema.statics.findByNameCI = function (name) {
//   return this.findOne({ name: { $regex: new RegExp(`^${name}$`, 'i') } });
// };

// /**
//  * Seed default plans — creates or updates Basic/Standard/Premium
//  * @param {Array<Object>} customPlans - optional array of plan objects with keys: name, price, amount, features, billingCycle
//  */
...
//   // Merge customPlans into defaults if provided
...
//     if (existing) {
//       // Update fields that might change (price/amount/features/isActive/billingCycle/duration)
